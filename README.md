...
3. 在电脑随机生成短信，或点击“上传 XML 并追加”导入短信文件，再编辑发送人、正文和接收时间。XML 支持 `<smses><sms address="发送人" body="内容" date="毫秒时间戳" type="1" /></smses>` 格式，仅支持收件短信。保留原顺序与内容，全部校验通过并保存后才追加；失败不改动原列表。单文件最多 64 MiB，列表最多 100,000 条。XML 在浏览器解析，点击发送后按原流程分批传输，当前版本需要电脑端与 APK 同时更新至 v1.3.0。
...
1. 手机保持工具在前台，授予读取短信权限。读取完整列表需临时设为默认短信应用，否则 Android 可能仅返回收件和已发送短信；清空全部必须在准备预览前取得默认角色。电脑在“手机短信管理”选择一台手机，点击读取短信。
2. 每页展示 50 条短信，包含发送人、正文、时间、类型及 read/status/locked。支持按发送人精确匹配、内容关键词字面包含、时间范围、已读状态、状态码及锁定状态组合筛选。默认排除锁定短信，可手动纳入。
3. 可以逐条勾选、选择当前页，或选择全部筛选结果。切换手机或筛选条件会清除选择，避免误用旧范围。短信读取结果与电脑的导入草稿独立。
4. 发起删除后，手机固定待删记录，展示数量和样例；临时设为默认短信应用，确认一次后每批最多 200 条依次删除，显示累计进度。手机可以取消待确认的删除任务。
5. “一键清空所有短信”忽略筛选，包含锁定短信以及收件箱、已发送、草稿等系统 SMS 记录，不包含 MMS 彩信。手机确认弹窗会明确说明范围和不可恢复。

删除只作用于手机预览时固定的记录，不会把之后新收到的短信纳入。勾选记录在读取后发生变化、删除前记录变化或消失、权限变化、网络同步失败、离开前台时会停止；已经删除的记录不会恢复。手机进程中断后不会自动继续删除；网络恢复仅同步结果。短信删除与本地进度记录无法跨数据库原子提交，中断时已确认删除数可能小于实际删除数。

管理请求会持久保存请求 ID，断网或刷新后重试同一个请求，不自动创建另一份删除任务。读取的短信页面及删除预览会传回当前配对电脑并保存在服务数据中；仅在可信测试网络使用。不会把读取结果发送到业务上报接口。
...
公开安装包见 [GitHub Releases](https://github.com/allenwangzero/android-sms-test/releases)。手机管理使用协议 v4，需要电脑端与 APK 同时升级至 v1.3.0 或更新版本；旧 APK 无法获取新服务中的任务。已有电脑端数据库会保留，升级前请完成或停止正在执行的手机任务。
...
node scripts/test-sms-manager.cjs /tmp/sms-pending-test/node_modules/jsdom /tmp/sms-pending-test/node_modules/fake-indexeddb
```
...
8. 管理端读取并跨页勾选，切换设备/筛选后选择应清空；拒绝读取权限时明确失败。
9. 用无真实数据的测试机验证勾选删除、筛选删除、含锁定短信的清空；确认后新收短信必须保留，改变待删短信应停止，拒绝/取消/重启不得自动删除。
10. 中途断开 Wi-Fi 或退出前台，确认停止整个任务；恢复网络只同步结果，不自动续写。电脑上传到一半刷新页面，重试应只补充缺失批次，并且仅创建一组任务。
