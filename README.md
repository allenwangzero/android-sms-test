...
3. 在电脑随机生成短信，或点击“上传 XML 并追加”导入短信文件，再编辑发送人、正文和接收时间。XML 支持 `<smses><sms address="发送人" body="内容" date="毫秒时间戳" type="1" /></smses>` 格式，仅支持收件短信。保留原顺序与内容，全部校验通过并保存后才追加；失败不改动原列表。单文件最多 64 MiB，列表最多 100,000 条。XML 在浏览器解析，点击发送后按原流程分批传输，当前版本需要电脑端与 APK 同时更新至 v1.4.0。
...
系统短信写入与本地批次记录不能跨数据库原子提交：写入后、记录前进程中断时，最后一条可能未被记录，批次清理不会推断或补删这条短信。保存批次记录失败立即停止后续导入。此功能需电脑端和 APK 同步升级至 v1.4.0。
...
管理页还提供“下载安装 APK”二维码。将 `android-sms-test-v1.4.0.apk` 放入项目根目录的 `dist/`，手机用相机或浏览器扫码即可从电脑直接下载，无需访问 GitHub。Compose 已将 `./dist` 只读挂载至 `/app/dist`；Python 启动同样读取项目的 `dist/`。安装包缺失时页面会提示，不会跳转外网；安装包不会随 Git 仓库下载，需自行放入。
...
公开安装包见 [GitHub Releases](https://github.com/allenwangzero/android-sms-test/releases)。手机管理使用协议 v5，需要电脑端与 APK 同时升级至 v1.4.0 或更新版本；旧 APK 无法获取新服务中的任务。已有电脑端数据库会保留，升级前请完成或停止正在执行的手机任务。
