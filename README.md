...
3. 在电脑随机生成短信，或点击“上传 XML 并追加”导入短信文件，再编辑发送人、正文和接收时间。XML 支持 `<smses><sms address="发送人" body="内容" date="毫秒时间戳" type="1" /></smses>` 格式，仅支持收件短信。保留原顺序与内容，全部校验通过并保存后才追加；失败不改动原列表。单文件最多 256 MiB，列表最多 100,000 条。XML 在浏览器解析，点击发送后按原流程分批传输，当前版本需要电脑端与 APK 同时更新至 v1.5.0。
...
“短信读取与删除”的目标手机下拉列表显示在线/离线，并展示选中设备最后在线时间；判定与写入设备列表一致，超过 10 秒未收到心跳显示离线。服务或网络中断也会使状态随最后心跳过期。

写入设备列表和管理模块选中设备旁均可“移除”。确认后移除电脑保存的配对并使旧设备令牌失效，结束该设备未完成的导入/短信管理任务；包含该设备的未提交上传会整次取消，不会悄悄改变原目标设备集合。已经提交给其他手机的任务继续保留，历史结果不删除。

移除不会删除手机短信，也不会立即遥控停止已开始的本地操作；手机将在后续连接失败时停止，已发生的操作不回滚，电脑进度保留最后已确认数量。重新使用需扫码配对，作为新设备连接；此前按旧设备身份保存的批次清理记录和导出关联不会转移到新配对。只想暂时断开手机时，无需移除配对。

## 导出与跨设备复现收件短信

1. 在“短信读取与删除”选择源手机，可导出勾选短信、全部筛选结果，或“全部收件短信”（包含锁定）。完整导出需要读取权限和默认短信应用角色；手机保持前台。现有筛选默认排除锁定，勾选含其他短信类型时会明确拒绝，不会静默漏掉。
2. 手机读取完整字段并冻结快照，按每 500 条上传；页面显示已传数量。只有完整成功才允许下载 XML / JSON，失败不能下载半份数据。导出不修改或删除源手机短信；刷新可恢复任务和最近完成文件。
3. 在“短信写入”先清空电脑草稿，导入导出的 XML / JSON 并检查数量和字段。请先完成文件校验，再对目标手机执行“一键清空所有短信”，在目标手机确认并等待清空成功。
4. 选中目标手机，发送同一份列表并在手机确认导入；多台目标手机可复用同一份文件。导入完成后可再次导出核对。

一致范围是收件短信的 sender、body、timestamp、type、protocol、subject、service_center、read、status、locked、seen、date_sent；系统短信 ID、会话 ID、SIM 卡关联等不复制，MMS、已发送、草稿等不在本版导出/导入范围。目标清空会删除所有 SMS 类型，而导入只恢复收件短信，操作前应确认这符合测试需求。清空后若目标收到新短信，或系统/其他应用改动字段，也会导致差异；本工具不持续同步。

导出读取原始全文，不使用截断后的网页预览。无法按现有导入限制表示的记录（例如空地址/正文、超长内容、非法字段或不支持的空值）会明确报错并停止，不替换数据或默默丢弃。每次最多 100000 条，完整规范 JSON 最多 256 MiB；文件下载和导入也最多 256 MiB。XML 无法表示的控制字符或转义后超限时可改用 JSON。支持值为文本 `null` 与真正空值的无损区分。

手机端临时导出快照在终态可靠保存后回收，异常遗留快照在启动时重试回收；已完成文件保存在电脑服务数据中以便重新下载。应用重启后不自动重新导出，网络恢复仅同步已有结果。

## 按导入批次清理

在“短信读取与删除”模块选择手机，点击“读取导入批次”，选择需要清理的一次导入任务。每个批次对应一次完整导入任务（不是传输时的 500 条分块），展示导入时间、计划条数、已记录条数及状态；失败或中断任务中已记录的短信也可清理。手机预览目标并确认后执行，显示总进度，失败停止。

批次记录从 v1.4.0 开始保存在手机应用的私有数据库中，按配对设备 ID 和任务 ID 隔离，重新连接同一电脑数据库不会丢失。旧版本导入未记录实际短信 ID，不能追溯清理；卸载工具或清除应用数据也会丢失记录。

清理仅使用当时记录的短信 ID 和完整字段指纹，不按发送人或内容猜测归属。准备时已不存在的短信和字段已变化的短信（包括 read、status、locked）会跳过，并在确认中展示数量；确认后记录再发生变化则停止。已记录条数是导入时保存的历史条数，不代表当前仍可删除数量；重复清理会跳过已经删除的短信。锁定状态未发生变化的本批次短信也属于清理范围。

系统短信写入与本地批次记录不能跨数据库原子提交：写入后、记录前进程中断时，最后一条可能未被记录，批次清理不会推断或补删这条短信。保存批次记录失败立即停止后续导入。此功能需电脑端和 APK 同步升级至 v1.5.0。
...
管理页还提供“下载安装 APK”二维码。将 `android-sms-test-v1.5.0.apk` 放入项目根目录的 `dist/`，手机用相机或浏览器扫码即可从电脑直接下载，无需访问 GitHub。Compose 已将 `./dist` 只读挂载至 `/app/dist`；Python 启动同样读取项目的 `dist/`。安装包缺失时页面会提示，不会跳转外网；安装包不会随 Git 仓库下载，需自行放入。
...
公开安装包见 [GitHub Releases](https://github.com/allenwangzero/android-sms-test/releases)。手机管理使用协议 v6，需要电脑端与 APK 同时升级至 v1.5.0 或更新版本；旧 APK 无法获取新服务中的任务。已有电脑端数据库会保留，升级前请完成或停止正在执行的手机任务。
...
- XML 中的 `protocol`、`type`、`subject`、`service_center`、`read`、`status`、`locked`、`seen`、`date_sent` 会随原始短信保存、传输并写入 Android；继续仅接受 `type="1"` 收件短信。电脑列表与手机确认预览展示 `read`、`status`、`locked` 的含义和数值。
- 未提供元数据时默认 `type=1`、`protocol=0`、`read=1`、`status=-1`、`locked=0`，`subject` 和 `service_center` 为 null。XML 中可空属性的 `"null"` 转为真正空值；缺失 seen 时与 read 一致，缺失 date_sent 时为 0；显式提供时分别保留。
